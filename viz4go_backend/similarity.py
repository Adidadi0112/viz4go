...
def information_content(G, term):
    ...
    if term not in G.lower_bounds:
        raise (f"Base exception class Missing term: {term}")
...
def lowest_common_ancestor(G, term1, term2):
    ...
    G.require("Pre-calculated lower bounds")
    if term1 not in G:
        raise (f"Invalid GoGraph lookup (ex. missing node or edge) Missing term: {term1}")
    if term2 not in G:
        raise (f"Invalid GoGraph lookup (ex. missing node or edge) Missing term: {term2}")
